...
## Executing program
There are three types of jobs that can be submitted depending on the purpose of the run:

### Testing
Runs 20 steps to verify the full pipeline (data staging, model loading, training, checkpoint upload) without committing to a full run.

```bash
sbatch training/finetune/ag-vla-finetune-test.slurm
```

### Single run
Full training run with the default config (or with CLI overrides).

```bash
sbatch training/finetune/ag-vla-finetune-single.slurm
```

### Hyperparameter sweep
Submits a SLURM job array — one task per config. Edit the `RANKS`, `ALPHAS`, `DORAS`, `LRS`, `BS` arrays in the script to define the sweep configs, and update `--array=0-N` to match.

```bash
sbatch training/finetune/ag-vla-finetune-sweep.slurm
```
...
TODO: Talk about training and validation metrics here
